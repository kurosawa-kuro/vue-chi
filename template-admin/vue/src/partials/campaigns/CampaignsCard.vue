        ...
        <div class="flex items-center justify-between">
          ...
          <div class="flex shrink-0 -space-x-3 -ml-px">
            <router-link
              ...
              :to="member.link"
            >
              <img class="rounded-full border-2 border-white dark:border-gray-800 box-content" :src="member.image" width="28" height="28" :alt="member.name" />
            </router-link>
          </div>
        </div>
      </header>
      <div class="grow mt-2">
        <router-link class="inline-flex text-gray-800 dark:text-gray-100 hover:text-gray-900 dark:hover:text-white mb-1" :to="item.link">
          <h2 class="text-xl leading-snug font-semibold">{{item.title}}</h2>
        </router-link>
        <div class="text-sm">{{item.content}}</div>
      </div>
      <footer class="mt-5">
        <div class="text-sm font-medium text-gray-500 mb-2">{{item.dates.from}} <span class="text-gray-400 dark:text-gray-600">-&gt;</span> {{item.dates.to}}</div>
        <div class="flex justify-between items-center">
          ...
          <div>
            <router-link class="text-sm font-medium text-violet-500 hover:text-violet-600 dark:hover:text-violet-400" :to="item.link">View -&gt;</router-link>
          </div>
        </div>
      </footer>
    </div>
  ...
  setup() {
    ...
    const categoryIcon = (category) => {
      switch (category) {
        ...
        case '4':
          return (
            `<div class="w-10 h-10 rounded-full flex items-center justify-center shrink-0 bg-violet-500">
              <svg class="w-9 h-9 fill-current text-white" viewBox="0 0 36 36">
